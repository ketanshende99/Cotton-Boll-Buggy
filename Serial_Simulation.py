# -*- coding: utf-8 -*-
"""
Created on Fri Aug 23 16:20:52 2024
...
def simulate_radio_communication(harvester, base_station, serial_conn):
    data = harvester.get_gps_data()
    base_station.receive_data(data)
    
    # Send data to the serial terminal
    data_str = f"{data['latitude']},{data['longitude']},{data['bin_level']},{data['trigger']}\n"
    serial_conn.write(data_str.encode())  # Send as bytes
...
# Initialize serial communication (adjust parameters as necessary)
serial_conn = serial.Serial('/dev/ttyUSB0', 9600, timeout=1)  # Replace with your port and settings
...
def update(frame):
    if boll_buggy.is_moving:
        ...
        harvester.move()
        data = simulate_radio_communication(harvester, base_station, serial_conn)
        if data['trigger']:
